...
ScaleObs 自动发现 Headscale / Tailscale 组网节点，部署不足 20MB 的极简 Agent 采集主机与容器状态，单页面展示全网健康状态、AI 工作负载标记，异常醒目高亮，作为 Grafana、Portainer 互补顶层门户，而非替代品。
...
在多机容器化微服务并行开发场景下，常会遇到三类运维难题：

**微服务分散，故障定位低效**  
多个业务服务分布在云服务器、本地小主机、边缘设备上。某个容器异常宕机时，没有全局总览视图，只能逐个 SSH 登录机器执行 `docker ps` 排查，耗时繁琐。

**运维工具割裂，远程管理杂乱**  
查看指标用 Grafana、管理容器用 Portainer、维护内网节点用 Headscale，排查问题需要频繁切换多个浏览器标签，地址与账号混乱；新增节点还要手动在各个平台添加接入配置，重复性工作多。

**跨组网网络干扰，真假故障难分辨**  
基于 Tailscale / Headscale 跨地域组网时，隧道抖动、链路超时很常见。经常分不清是服务真崩溃，还是临时网络波动，容易误重启、误处置。

传统 Prometheus + cAdvisor 监控栈组件臃肿、内存占用高，低配边缘节点部署压力大，新增节点配置流程繁琐，并不适配个人 / 小规模集群场景。

### 项目定位

ScaleObs 不会替代 Grafana、Portainer、Headscale 任意专业工具，而是搭建**顶层统一总览入口**：
...
各节点部署不足 20MB 的极简 Agent，只读采集主机资源与 Docker 容器状态；对接 Headscale 实现节点入网自动发现；单页面展示全网健康状态、容器运行情况、AI 工作负载标记，异常醒目高亮；发现问题可一键跳转对应后台深度排查，用极低开销解决分布式集群观测碎片化问题。
...
- **超轻量 Agent** — 单二进制无额外依赖，常驻内存约 20MB，适配低配边缘机型
- **自动发现** — 对接 Headscale / Tailscale，节点加入内网自动识别、自动上报，免手动配置接入
- **资源监控** — 实时采集整机 CPU、内存、磁盘、网络，同步读取全部容器运行状态
- **AI 编程负载识别** — 独有功能，对 OpenCode、Claude Code 类 Agent 容器做专属标签标记
- **全局异常高亮** — 快速发现宕机、OOM、反复重启等异常实例
- **一键跳转链路** — 跳转 Grafana 看趋势、Portainer 操作容器、Headscale 调整组网
- **配置极简** — 仅依靠单份 YAML 文件完成全部对接设定

## 快速部署

每台被管理节点运行 ScaleObs Agent，挂载 docker.sock 实现容器只读采集。部署 ScaleObs 服务端，配置对接你的 Headscale 地址。打开面板即可自动收拢全网所有节点，开箱即用总览运维。
